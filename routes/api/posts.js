...
router.get(
  ...
  (req, res) => {
    Post.find({ author: req.user.user_name })
      ...
      .catch(err =>
        res.status(400).json({ user: 'Error fetching posts of user' })
      );
  }
);
...
router.get('/author/:author', (req, res) => {
  Post.find({ author: req.params.author })
    .then(posts => res.status(200).json(posts))
    .catch(err =>
      res.status(400).json({ author: 'Error fetching posts of author' })
    );
});
...
router.delete(
  ...
  (req, res) => {
    const author = req.user.user_name;
    Post.findByIdAndDelete({ author, _id: req.params.id })
      .then(doc => res.status(200).json(doc))
      .catch(err => res.status(400).json({ delete: 'Error deleting ea post' }));
  }
);
